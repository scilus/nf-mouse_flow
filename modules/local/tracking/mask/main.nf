process TRACKING_MASK {
    ...
    """
    scil_volume_math convert $ano $ano --data_type int16 -f
    scil_labels_combine ${prefix}__ventricles_mask.nii.gz \
        --volume_ids $ano $ventricules_labels_ids \
        --merge_groups -f
    scil_labels_combine ${prefix}__seeding_mask.nii.gz \
        --volume_ids $ano $seeding_labels_ids \
        --merge_groups -f

    scil_volume_math convert ${prefix}__ventricles_mask.nii.gz  ${prefix}__ventricles_mask.nii.gz --data_type uint8 -f
    scil_volume_math subtraction $mask ${prefix}__ventricles_mask.nii.gz ${prefix}__tracking_mask.nii.gz --data_type uint8 -f
